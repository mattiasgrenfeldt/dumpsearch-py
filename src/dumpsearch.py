...
import parseformat, guesser, exporter
...
def parse():
    ...
    parser.add_argument('-c', "--config", help="DB config file.")
    parser.add_argument('-o', "--outfile", help="File to export to")
    ...
    if args.outfile == None and args.config == None:
        print("[ERROR] You have to specify either a DB config file (-c) or an outfile (-o).")
        sys.exit(1)
    elif args.outfile != None:
        exp = exporter.FileExporter(args.outfile)
    else:
        exp = exporter.DBExporter(args.config)
